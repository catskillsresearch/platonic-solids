...
This paper addresses these questions through an analytical exposition coupled with a fully verified formal certificate in Lean 4. Figure \ref{fig:overview} is the coarsest dependency blueprint: topology bounds the $3$D census, while the sign of the Schläfli determinant $\Delta$ bounds the $4$D census.

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{figures/overview.pdf}
\caption{Coarse blueprint of the two classifications. In dimension $3$, $\chi(S^2)=2$ and regularity incidence force $(p-2)(q-2)<4$ and the five Platonic solids. In dimension $4$, $\chi(S^3)=0$ supplies no flag bound; the six regular polychora are the Platonic-compatible triples with $\Delta>0$.}
\label{fig:overview}
\end{figure}
...
\caption{Finer blueprint of $\mathtt{PlatonicSolids/SingularExcision/}$. Affine subdivision, mesh geometry, and the prism homotopy assemble the small-chain retract; cover chains are identified with small singular chains; the remaining module is the open-cover quasi-isomorphism.}
...
| `PlatonicSolids/SingularExcision/*.lean` | §2.1 | Mesh shrinking, prism homotopy, small retract, cover $=$ small chains |
