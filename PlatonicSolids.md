...
4. **An explicit finite comparison, not a general one:**
   Closed-arc singular chains on $|\partial\Delta[2]|$ do not form a short exact sequence. The compared proof therefore enlarges the last edge and horn to punctured-circle open neighborhoods, uses deformation retractions onto those neighborhoods and onto the two-point intersection, and applies open-cover excision. The result is recorded as a Palomar theorem for $\partial\Delta[2]$ only.
